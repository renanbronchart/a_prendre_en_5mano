jQuery(document).ready(function () {
	...
	jQuery('#buttons_size_list input').click(function() {
		...
		if (thisval == "20") {
			jQuery('#buttons_size_block h4').html('Small');
		}
		else if (thisval == "30") {
			jQuery('#buttons_size_block h4').html('Medium');
		}
		else {
			jQuery('#buttons_size_block h4').html('Big');
		}
		...
		jQuery(".huge-it-share-buttons li a").each(function() {
			...
			jQuery(this).css({
				'background-position': '-' + strleft + 'px -' + strtop + 'px',
				'width': thisval,
				'height': thisval
			});
			i++;
	...
	jQuery('.close_free_banner').on('click', function() {
		...
		hgShareSetCookie('hgSliderFreeBannerShow', 'no', {expires: 3600});
	});
});

function hgShareSetCookie(name, value, options) {
	options = options || {};
